...
export const SidebarLink = ({
  ...
  useEffect(() => {
    const checkActive = () => {
      ...
      // Special case handling for profile, settings, discover, and favorites
      if (link.href === '/profile' && pathname.startsWith('/profile/')) {
        setIsActive(true);
        return;
      }
      
      // Exact path matching for settings, discover, saved, and favorites
      if (['/settings', '/discover', '/saved', '/favorites'].includes(link.href)) {
        setIsActive(pathname === link.href);
        return;
      }
      
      // For other routes, use the existing logic but make it more precise
